...
describe('Functions & Errors with Wrapped Asset', () => {
    let Bridge;
    let bridge;
    ...
    it('- should revert locking if there is not enough allowance', async () => {
        await expect(bridge.connect(executor).lock(assetID, amount, targetChain)).to.be.revertedWith('ERC20: burn amount exceeds allowance');
    });
